...
public class AlbumDao extends JdbcDaoSupport {
	...
	public Album loadAlbum(Long id) throws DataAccessException {
		return getJdbcTemplate().queryForObject(LOAD_QUERY,
				AlbumRowMapper.INSTANCE, new Object[] { id });
	}
	
	public List<Album> findAlbumByArtistId(Long artistId)
			throws DataAccessException {
		return getJdbcTemplate().query(FIND_BY_ARTIST_ID_QUERY,
				AlbumRowMapper.INSTANCE, new Object[] { artistId });
	}
	
	public List<Album> findAlbumByNameLike(String name)
			throws DataAccessException {
		String likeifiedName = "%"
				+ StringHelper.escapeLikeWildcards(name).toLowerCase() + "%";
		return getJdbcTemplate().query(FIND_BY_ALBUM_NAME_QUERY,
				AlbumRowMapper.INSTANCE, new Object[] { likeifiedName });
	}
